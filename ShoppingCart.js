...
function CartAdd(obj, type, producttype, product_id, product_quantityInput, product_number, product_price) {
    ...
    if (parseInt(product_quantity) > parseInt(product_number)) {
        $.post("http://www.yabis.com.cn/Member/Ajax/LanguageHandler.ashx", { action: "loadLanguage", langus: "购买商品失败;商品添加到购物车失败;购买数量不能大于库存哦", cache: Math.random() }, function (langu) {
    ...
    $.ajax({
        type: "post",
        url: "http://www.yabis.com.cn/Ajax/ShoppingCartHandler.ashx",
        ...
        success: function (data, textStatus) {
            if (data.msg == 1) {
                if (type == 1) {
                    location.href = "http://www.yabis.com.cn/Member/BuyNow.aspx";
                } else {
                    var HintHtml = '<div id="cart_info" class="ctDia-buyTipBox clearfix" style="width:337px">'
						...
						+ ' <div class="ctDia-buyTxtHand"><a class="goto-accountBtn" title="' + data.gotoCart + '" href="http://www.yabis.com.cn/Member/Cart.aspx"></a>'
...
function LoadLogin(product_id, product_quantity, producttype) {
    $.post("http://www.yabis.com.cn/Member/Ajax/LanguageHandler.ashx", {
        action: "loadLanguage",
    ...
    , cache: Math.random()
        ...
        strhtml += '<div class="paraitem ct-dialoginHand clearfix"><a href="http://www.yabis.com.cn/Member/Register.aspx" class="fl">' + langu.注册 + '</a> <a href="http://www.yabis.com.cn/Member/ForgotPwd.aspx" class="fr">' + langu.忘记密码 + '？</a></div>';
        ...
        $("#subInfo").html($('<input/>').attr("type", "submit").attr("value", langu.登录2).addClass("dia-loginbtn").click(function () {
            ...
            $.post("http://www.yabis.com.cn/Member/Ajax/LoginHandler.ashx", {
...
function LoadTableInfo(ProductId) {
    $.post("http://www.yabis.com.cn/Member/Ajax/LanguageHandler.ashx", { action: "loadLanguage", langus: "真实姓名;公司名称;联系手机;联系邮箱;期望价格;了解内容;询价提交", cache: Math.random() }, function (langu) {
        ...
        $("#subInfo").html($('<input/>').attr("type", "submit").attr("value", langu.询价提交).addClass("zdyform-subtn").click(function () {
            ...
            $.post("http://www.yabis.com.cn/Member/Ajax/EnquiryHandler.ashx", {
